...
const String sFieldNotFound = "Field not found.";
  ...
  // CallbackResult - holds result of anonymous method called by MessageDlg
...
void __fastcall TNotesClientForm::ActionEditSaveExecute(TObject *Sender)
{
  struct TCloseDialogHandler : public TCppInterfacedObject<TInputCloseDialogProc> {
	System::Uitypes::TModalResult CallbackResult;
	void __fastcall Invoke(const System::Uitypes::TModalResult AResult) {
	  CallbackResult = AResult;  // store return value in global
	}
  };
  _di_TInputCloseDialogProc handler = new TCloseDialogHandler();

  FBindSourceAdapter->Post();
  MessageDlg(String()<< GetTitleField()->GetTValue().ToString() + " saved",
	TMsgDlgType::mtInformation, TMsgDlgButtons() << TMsgDlgBtn::mbOK, 0, handler);
  PopView();
}
//---------------------------------------------------------------------------
...
void __fastcall TNotesClientForm::ActionEditCancelExecute(TObject *Sender)
{
  ...
  _di_TInputCloseDialogProc handler = new TCloseDialogHandler();
  MessageDlg("Cancel changes?", TMsgDlgType::mtConfirmation, mbOKCancel, 0, handler);
  if ((!FBindSourceAdapter->Modified) || (CallbackResult != mrCancel)) {
