...
const String HRDeviceName = "Cardiosport HRM";
const String ServiceUUID = "";
const String CharactUUID = "";
...
//---------------------------------------------------------------------------
void __fastcall TfrmHeartMonitor::FormShow(TObject *Sender)
{
	FBLEManager = TBluetoothLEManager::Current;
	FBLEManager->OnDiscoveryEnd = DoDiscoveryEndEvent;
}
//---------------------------------------------------------------------------
void __fastcall TfrmHeartMonitor::DoServicesDiscovered(System::TObject* const Sender,
...
//---------------------------------------------------------------------------
THRMFlags * _fastcall TfrmHeartMonitor::GetFlags(Byte Data)
{
	...
	switch(LValue) {
		case 2 : _return->SensorContactStatus = NonDetected;
			break;
		case 3 : _return->SensorContactStatus = Detected;
			break;
		default:
			_return->SensorContactStatus = NonSupported;
			break;
	}
	_return->EnergyExpended = (((Data & ENERGY_EXPANDED_STATUS_MASK) >> 3) == 1);
	_return->RRInterval = (((Data & RR_INTERVAL_MASK) >> 4) == 1);
	return _return;
}

void __fastcall TfrmHeartMonitor::ManageCharacteristicData(const TBluetoothGattCharacteristic *ACharacteristic)
{
	if (const_cast<TBluetoothGattCharacteristic*>(ACharacteristic)->UUID == HRMEASUREMENT_CHARACTERISTIC) {
		DisplayHeartRateMeasurementData(const_cast<TBluetoothGattCharacteristic*>(ACharacteristic)->Value);

	}

	if(const_cast<TBluetoothGattCharacteristic*>(ACharacteristic)->UUID == BODY_SENSOR_LOCATION_CHARACTERISTIC) {
		DisplayBodySensorLocationData(const_cast<TBluetoothGattCharacteristic*>(ACharacteristic)->Value[0]);
	}
}
...
//---------------------------------------------------------------------------
void __fastcall TfrmHeartMonitor::DoDiscoveryEndEvent(System::TObject* const Sender,
	TBluetoothLEDeviceList* const ADeviceList)
{
	...
	FBLEDevice = NULL;
	for(int i = 0; i < FBLEManager->LastDiscoveredDevices->Count; i++) {
		if(FBLEManager->LastDiscoveredDevices->Items[i]->DeviceName == HRDeviceName) {
   ...
   //		FBLEDevice->OnCharacteristicChanged = DoCharacteristicChanged;
...
//---------------------------------------------------------------------------
void __fastcall TfrmHeartMonitor::DoDescriptorRead(System::TObject* const Sender,
	TBluetoothGattDescriptor* const ADescriptor, TBluetoothGattStatus AGattStatus)
{
	if(AGattStatus != TBluetoothGattStatus::Success) {
		Memo1->Lines->Add("Error reading Characteristc " + ADescriptor->UUIDName  + ": " +
...
void __fastcall TfrmHeartMonitor::EnableHRMMonitorize(bool Enabled)
{
	...
	if(FHRMeasurementGattCharact != NULL) {
		...
		FBLEDevice->OnDescriptorRead = DoDescriptorRead;
		FBLEDevice->WriteDescriptor(LDescriptor);
#endif
		FBLEDevice->SetCharacteristicNotification(FHRMeasurementGattCharact, Enabled);
...
void __fastcall TfrmHeartMonitor::DoScan(void)
{
  ...
  lblBodyLocation->Text = "";
  lblContactStatus->Text = "";
  FBLEManager->StartDiscovery(2500);
}
//---------------------------------------------------------------------------
