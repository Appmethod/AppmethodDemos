...
const String CProximityDeviceName = "TX POWER";
...
void __fastcall TfrmProximityForm::DoDiscoveryEndEvent(TObject *Sender, TBluetoothLEDeviceList *ADeviceList)
{
  ...
  for (I = 0; I <= FBLEManager->LastDiscoveredDevices->Count - 1; I++)
  {
	...
	if (FBLEDevice->GetService(StringToGUID("{00001804-0000-1000-8000-00805F9B34FB}")) != NULL)
	{
...
void __fastcall TfrmProximityForm::WriteImmediateAlertLevel(Byte AlertLevel)
{
  TBytes LData;
...
void __fastcall TfrmProximityForm::WriteLinkLossAlertLevel(Byte AlertLevel)
{
	TBytes	LData;
	LData.set_length(1);
	LData[0] = AlertLevel;
