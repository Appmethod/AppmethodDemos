...
System::DynamicArray<System::UnicodeString>__fastcall TMusicPlayer::GetAlbums(void) {
	...
	for (int i = 0; i < query->collections()->count(); i++) {
		_di_MPMediaItemCollection item = TMPMediaItemCollection::Wrap(query->collections()->objectAtIndex(NativeUInt(i)));
		FAlbums[i].Name =
			TNSString::Wrap(item->representativeItem()->valueForProperty(MPMediaItemPropertyAlbumTitle()))->UTF8String();
		FAlbums[i].Artist =
			TNSString::Wrap(item->representativeItem()->valueForProperty(MPMediaItemPropertyArtist()))->UTF8String();
		FAlbums[i].Album_ID = i;
		if (item->valueForProperty(MPMediaItemPropertyArtwork()) != nullptr) {
			auto artwork = TMPMediaItemArtwork::Wrap(item->valueForProperty(MPMediaItemPropertyArtwork()));
			NSSize art_size;
			art_size.width = 200;
			art_size.height = 200;
			UIImage *art_img = artwork->imageWithSize(art_size);
			FAlbums[i].Artwork = UIImageToBitmap(art_img, 0, TSize(1024, 1024));
		}
		else {
