...
// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetSensorCategoryName
	(TSensorCategory ASensorCategory) {
	String _return = cND;
...
void __fastcall TfrmAboutSensors::ListBoxItemClick(TObject * Sender) {
	if (dynamic_cast<TListBoxItem*>(Sender)) {
		...
		if (FActiveSensor != NULL && FActiveSensor->Started == false)
			FActiveSensor->Start();
	}
	FShowInfo = true;
}
...
// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetFullInfo(String ACategory, String AType,
	String AClass, String AAvailableProperties) {
	return Format("\nCategory:\n   %s\n\nSensor type:\n   %s\n\nBase class:\n   %s\n\nAvailable properties:\n%s", 
                ARRAYOFCONST((ACategory, AType, AClass, AAvailableProperties)));
}

// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetTypeNameLocation
	(TLocationSensorType AType) {
	String Result;
	switch (AType) {
...
// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetTypeNameOrientation
	(TOrientationSensorType AType) {
	String Result = cND;
	switch (AType) {
...
// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetTypeNameElectro
	(TElectricalSensorType AType) {
	String Result = cND;
	switch (AType) {
...
__fastcall TfrmAboutSensors::TfrmAboutSensors(TComponent* Owner) : TForm(Owner),
  cBorder(10), cND("Not defined"), cForm(L"  %s =\n%30s        %3.5f \n"),
		   cFormS(L"  %s =\n%30s        %s \n")
  {}
// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetInfoAboutLocation(TCustomSensor *ASensor)
{
	...
	if (ls->AvailableProperties.Contains
	...
	return GetFullInfo(GetSensorCategoryName(ASensor->Category),
		GetTypeNameLocation(ls->SensorType), ls->ClassName(), LValues);
}

// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetInfoAboutEnv(TCustomSensor *ASensor) {
	String LValues = "";
	TCustomEnvironmentalSensor * ls = static_cast<TCustomEnvironmentalSensor*>
		(ASensor);
	if (ls->AvailableProperties.Contains
	...
	return GetFullInfo(GetSensorCategoryName(ASensor->Category),
		GetTypeNameEnv(ls->SensorType), ls->ClassName(), LValues);
}
...
String __fastcall TfrmAboutSensors::GetInfoAboutMotion(TCustomSensor *ASensor) {
	...
	if (ls->AvailableProperties.Contains(TCustomMotionSensor::TProperty::Speed))
		LValues += ToFormStr("Speed", ls->Speed);
	return GetFullInfo(GetSensorCategoryName(ASensor->Category),
		GetTypeNameMotion(ls->SensorType), ls->ClassName(), LValues);
}
...
String __fastcall TfrmAboutSensors::GetInfoAboutOrientation(TCustomSensor *ASensor) {
	 ...
	 return GetFullInfo(GetSensorCategoryName(ASensor->Category),GetTypeNameOrientation(ls->SensorType),
...
String __fastcall TfrmAboutSensors::GetInfoAboutMechanical(TCustomSensor *ASensor) {
	 ...
	 return GetFullInfo(GetSensorCategoryName(ASensor->Category), GetTypeNameMech(ls->SensorType),
		ls->ClassName(), LValues);
}
...
String __fastcall TfrmAboutSensors::GetInfoAboutElectro(TCustomSensor *ASensor)
{
	 ...
	 return GetFullInfo(GetSensorCategoryName(ASensor->Category), GetTypeNameElectro(ls->SensorType),
		ls->ClassName(), LValues);
}

// ---------------------------------------------------------------------------
String __fastcall TfrmAboutSensors::GetInfoAboutBiometric
	(TCustomSensor *ASensor) {
	 String LValues = "";
	 ...
	 return GetFullInfo(GetSensorCategoryName(ASensor->Category), GetTypeNameBio(ls->SensorType),
		ls->ClassName(), LValues);
}
...
String __fastcall TfrmAboutSensors::GetInfoAboutLight(TCustomSensor *ASensor) {
	 ...
	 return GetFullInfo(GetSensorCategoryName(ASensor->Category),GetTypeNameLight(ls->SensorType),
		ls->ClassName(), LValues);
}
...
String __fastcall TfrmAboutSensors::GetInfoAboutScanner(TCustomSensor *ASensor)
{
	 ...
	 return GetFullInfo(GetSensorCategoryName(ASensor->Category),GetTypeNameScanner(ls->SensorType),
		ls->ClassName(), LValues);
}
...
void __fastcall TfrmAboutSensors::lbMainItemClick(const TCustomListBox *Sender,
