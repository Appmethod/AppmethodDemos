...
uses
  ...
  FMX.Types, FMX.Controls, FMX.Forms, FMX.Graphics, FMX.Dialogs, FMX.StdCtrls, FMX.EditBox, FMX.SpinBox,
  FMX.Controls.Presentation, FMX.Edit, FMX.Layouts, FMX.ListBox, FMX.TabControl, System.Beacon, FMX.ListView.Types,
  FMX.ListView, BeaconsRender, FMX.Objects, System.Generics.Collections, FMX.ListView.Appearances;
...
begin
  ...
  begin
    ...
    begin
     if FBeaconManager.RegisterBeacon(TGUID.Create(EdGuid.Text), Trunc(sbMajor.Value), Trunc(sbMinor.Value)) then
        ListBox1.Items.Add(EdGuid.Text + ';' + Trunc(sbMajor.Value).ToString + ';' + Trunc(sbMinor.Value).ToString);
    end
    else
     if FBeaconManager.RegisterBeacon(TGUID.Create(EdGuid.Text), Trunc(sbMajor.Value)) then
       ListBox1.Items.Add(EdGuid.Text + ';' + Trunc(sbMajor.Value).ToString + ';' + Trunc(sbMinor.Value).ToString);
  end
  else
    if FBeaconManager.RegisterBeacon(TGUID.Create(EdGuid.Text)) then
      ListBox1.Items.Add(EdGuid.Text + ';' + Trunc(sbMajor.Value).ToString + ';' + Trunc(sbMinor.Value).ToString);
end;
...
begin
  ...
  begin
    ...
    if FBeaconManager.UnregisterBeacon(TGUID.Create(LStringGuid), LMajor, LMinor) then
