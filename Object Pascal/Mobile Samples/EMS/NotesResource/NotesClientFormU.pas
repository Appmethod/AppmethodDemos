...
uses
  ...
  FMX.Controls.Presentation, FMX.MobilePreview;
...
uses NotesAdapterModuleU, NotesClientModuleU, FMX.VirtualKeyboard, FMX.Platform, System.Math;
...
function TNotesClientForm.GetContentField: TBindSourceAdapterField;
begin
...
begin
  GetAdapter.Post;
  MessageDlg(Format('"%s" saved', [GetTitleField.GetTValue.ToString]),
...
var
  ...
  CallbackResult : TModalResult;
begin
     // Define anonymous method for MessageDlg callback

  ACallback := procedure(const AResult: TModalResult)
  begin
    CallbackResult := AResult;
  end;
  LTitle := GetTitleField.GetTValue.ToString;
  MessageDlg(Format('Delete "%s"?', [LTitle]),
    TMsgDlgType.mtConfirmation,
    mbOKCancel,
    0,
    ACallback
  );
  if CallbackResult <> mrCancel then
  begin
...
begin
  ...
  MessageDlg('Cancel changes?', TMsgDlgType.mtConfirmation, mbOKCancel, 0, ACallback);

  if (not FBindSourceAdapter.Modified) or (CallbackResult <> mrCancel) then
  begin
...
begin
  ...
  MessageDlg(Format('"%s" saved', [GetTitleField.GetTValue.ToString]), TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], 0,
      procedure(const AResult: TModalResult)
        begin
